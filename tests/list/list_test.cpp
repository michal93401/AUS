...
namespace tests
{
	...
	void ArrayListTestInsert::test()
	{
		...
		porovArray->add(400);
		porovArray->add(7777);
		SimpleTest::assertTrue(porovArray->equals(*testArray));
	...
	void DoubleLinkedListTestInsert::test()
	{
		...
		porovList->add(400);
		porovList->add(7777);
		SimpleTest::assertTrue(porovList->equals(*testList));
	...
	TestScenar::TestScenar(const char* testNazov)
		:SimpleTest(testNazov)
	{
	}

	void TestScenar::testScenar(std::string listType, float insert, float removeAt, float at, float getIndexOf)
	{
		srand((int)time(NULL));
		...
		structures::Logger::getInstance().logDuration(list->size(), timeCounter,
			listType + " : " + SimpleTest::getName());
		delete list;
	}

	TestScenarAal::TestScenarAal()
		:TestScenar("TestScenarAal")
	{
	}

	void TestScenarAal::test()
	{
		testScenar("ArrayList", 0.20, 0.20, 0.50, 0.10);
	}


	TestScenarBal::TestScenarBal()
		:TestScenar("TestScenarBal")
	{
	}

	void TestScenarBal::test()
	{
		testScenar("ArrayList", 0.35, 0.35, 0.20, 0.10);
	}


	TestScenarCal::TestScenarCal()
		:TestScenar("TestScenarCal")
	{
	}

	void TestScenarCal::test()
	{
		testScenar("ArrayList", 0.45, 0.45, 0.05, 0.05);
	}


	TestScenarAdll::TestScenarAdll()
		:TestScenar("TestScenarAdll")
	{
	}

	void TestScenarAdll::test()
	{		
		testScenar("DoubleLinkedList", 0.20, 0.20, 0.50, 0.10);
	}


	TestScenarBdll::TestScenarBdll()
		:TestScenar("TestScenarBdll")
	{
	}

	void TestScenarBdll::test()
	{
		testScenar("DoubleLinkedList", 0.35, 0.35, 0.20, 0.10);
	}


	TestScenarCdll::TestScenarCdll()
		:TestScenar("TestScenarCdll")
	{
	}

	void TestScenarCdll::test()
	{
		testScenar("DoubleLinkedList", 0.45, 0.45, 0.05, 0.05);
	}
	...
	void TestTimeInsertal::test()
	{
		...
		{
			number = rand() % 100;
			index = rand() % testArray->size();
			SimpleTest::startStopwatch();
			testArray->insert(number, index);
			SimpleTest::stopStopwatch();
	...
	void TestTimeInsertdll::test()
	{
		...
		{
			number = rand() % 100;
			index = rand() % testList->size();
			SimpleTest::startStopwatch();
			testList->insert(number, index);
			SimpleTest::stopStopwatch();
