...
namespace structures
{
	...
	void Heap<T>::push(int priority, const T& data)
	{
		...
		while (indexCurrent != 0 && list_->at(indexCurrent)->getPriority() <
									list_->at(indexParent)->getPriority()) 
		{
	...
	T Heap<T>::pop()
	{
		...
		if (index != -1) {
			...
			auto ptr = list_->removeAt(index);
			T data = ptr->accessData();
			delete ptr;
			...
			while (indexSon != -1 && list_->at(indexCurrent)->getPriority() >
	...
	inline int Heap<T>::getGreaterSonIndex(int index)
	{
		...
		if (indexLeft >= size) {
			...
			if (indexRight >= size) {
				...
				return list_->at(indexLeft)->getPriority() <
					list_->at(indexRight)->getPriority() ? indexLeft : indexRight;
			}
		}
	}
