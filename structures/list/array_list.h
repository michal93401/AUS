...
namespace structures
{
	...
	inline bool ArrayList<T>::equals(Structure& other)
	{
		...
		if (otherArray != nullptr) {
			if (size_ != otherArray->size()) return false;
			for (size_t i = 0; i < size_; i++) {
				if (array_->at(i) != otherArray->at(i)) return false;
			}
			return true;
		}
		return false;
		...
		// su rovnake?
		// velkost?
		// prvok po prvku v array_ => ak som narazil na iný prvok tak nie su rovnake
	}

	template<typename T>
	inline T& ArrayList<T>::at(int index)
	{
		Utils::rangeCheckExcept(index, size_, "nem dobrý index");
	...
	{
		Array<T>* newArray = new Array<T>(array_->size() * 2);
