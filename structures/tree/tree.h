...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::deepCopy()
	{
		...
		int numberOfProccesedSons = 0;
		for (int i = 0; numberOfProccesedSons < degree(); i++) {
			auto son = getSon(i);
			if (son != nullptr) {
				result->replaceSon(son->deepCopy(), i);
				numberOfProccesedSons++;
			}
		}
		return result;
	}
	...
	{
		if (this->isRoot()) { //parent_ == nullptr => je o kúsom rýchlejšie
			throw std::logic_error("I am groot!");
		}
	...
	inline void Tree<T>::PreOrderTreeIterator::populatePath(TreeNode<T>* current)
	{
		if (current != nullptr) {
			...
			int numberOfProccesedSons = 0;
			for (int i = 0; numberOfProccesedSons < current->degree(); i++) {
